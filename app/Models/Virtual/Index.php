...
class Index
{
    ...
    public function diff(Index $destination_index, Change $table_change)
    {
        ...
        foreach($source_attributes as $attribute_name => $attribute_value) {
            ...
            if($attribute_value != $destination_index->{$attribute_name}) {
                // Get the attributes
                $add_info = $source_index->getAttributes();
                // Create the SQL and add the change
                $parent_change->addChange($attribute_name, 'attribute_altered', 'attribute_index', Sql::alterIndex($this, $destination_index, $table_change->name), null, $add_info);
            }
        }
    ...
    /**
     * Get the attributes of this index
     * @return array
     */
